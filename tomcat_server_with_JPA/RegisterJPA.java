...
public class RegisterJPA extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request,HttpServletResponse response) throws ServletException,IOException{
    ...
    try {
        ...
        String stringQuery = "SELECT count(U) FROM UserData U where U.email= '"+email+"' and U.password= '"+password+"'";
        ...
        if(size> 0) {
          ...
          transaction = manager.getTransaction();
          transaction.begin();        
          int maxId = ((Number) manager.createQuery("SELECT max(U.id) FROM UserData U").getSingleResult()).intValue();
